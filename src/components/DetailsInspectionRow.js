...
const DetailsInspectionRow = ({inspection, formatDate, inspectionIndex, activeViolations, inspectionRowOnClick}) => {
  ...
  if(inspection.violation.length > 0) {
    ...
    violationRows = inspection.violation.map((item) => {
      return (
        <DetailsViolationRow violation={item} index={inspectionIndex} />
      );
    });
  }
  let inspectionRows = [];
  inspectionRows.push(
    <tr className={opts.rowClass} data-toggle={opts.dataToggle} data-target={opts.dataTarget} onClick={opts.clickHandler} key={inspection.inspection.inspectionSerialNum}>
